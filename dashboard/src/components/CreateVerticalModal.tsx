...
export default function CreateVerticalModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md animate-in fade-in duration-200">
      <div className="relative w-full max-w-2xl bento-card p-6 sm:p-8 border border-slate-200 shadow-2xl bg-white overflow-hidden max-h-[92vh] overflow-y-auto">
        {/* Ambient Gradient Mesh Orb */}
        <div className="absolute -top-16 -right-16 w-60 h-60 bg-gradient-to-bl from-indigo-500/10 via-purple-500/10 to-transparent rounded-full blur-3xl pointer-events-none" />
          ...
          <div className="flex items-center gap-3.5">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="px-2 py-0.5 rounded-full text-[10px] font-black uppercase bg-indigo-100 text-indigo-800 border border-indigo-200 font-mono">
                  Zero Code
                </span>
              </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
            {presetUseCases.map((p, idx) => (
              <button
                ...
                className="p-2.5 rounded-xl bg-slate-50 hover:bg-indigo-50 border border-slate-200 hover:border-indigo-300 text-left transition-all group cursor-pointer shadow-sm"
              >
                <span className="text-xs font-extrabold text-slate-900 group-hover:text-indigo-700 block truncate">
                  {p.name}
                </span>
        ...
        <form onSubmit={handleSubmit} className="mt-6 space-y-5 relative z-10">
          <div className="space-y-1.5">
            ...
            <input
              ...
              required
              disabled={loading}
              className="w-full bg-slate-50 border border-slate-200 focus:border-indigo-500 focus:bg-white rounded-2xl px-4 py-3 text-xs text-slate-900 placeholder-slate-400 outline-none transition-all font-sans font-medium shadow-inner"
            />
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <textarea
              ...
              required
              disabled={loading}
              className="w-full bg-slate-50 border border-slate-200 focus:border-indigo-500 focus:bg-white rounded-2xl p-3.5 text-xs text-slate-900 placeholder-slate-400 outline-none transition-all font-sans font-medium shadow-inner"
            />
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-1.5">
              ...
              <textarea
                ...
                rows={3}
                disabled={loading}
                className="w-full bg-slate-50 border border-slate-200 focus:border-indigo-500 focus:bg-white rounded-2xl p-3 text-xs text-slate-900 placeholder-slate-400 outline-none transition-all font-mono shadow-inner"
              />
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <textarea
                ...
                rows={3}
                disabled={loading}
                className="w-full bg-slate-50 border border-slate-200 focus:border-indigo-500 focus:bg-white rounded-2xl p-3 text-xs text-slate-900 placeholder-slate-400 outline-none transition-all font-sans font-medium shadow-inner"
              />
            </div>
          </div>
          ...
          <div className="pt-4 border-t border-slate-200 flex items-center justify-end gap-3">
            ...
            <button
              ...
              className="px-7 py-3 rounded-2xl bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-black shadow-md shadow-indigo-500/20 transition-all flex items-center gap-2 disabled:opacity-50 hover:scale-105 cursor-pointer"
            >
              {loading ? (
                <>
